...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    *
    ...
    // 2. 검색어 보내쓸 때 사용자 이름/이메일에 특정 문자열이 포함된 모든 사용자 n명 찾기
    ...
    * */
    ...
    // case2. @Query 어노테이션 사용  // select * from user where username = ?
    // - JpaRepository 인터페이스에 내장된 메서드만으로는 해결이 안되는 경우 raw query 작성
    // - 단, JPA는 테이블명이 아니라 객체 위주로 돌아가니 객체(Entity) 이름 사용
    // - nativeQuery  옵션
    // : 찐 SQL문을 사용하는 거라 DBMS에 종속적이게됨(ORM 특징 활용이 어려움)
    // ex) @Query(nativeQuery = true, value="select * from user"
